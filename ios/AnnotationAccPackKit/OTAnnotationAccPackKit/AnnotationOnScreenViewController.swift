...
class AnnotationOnScreenViewController: UIViewController {
    fileprivate let annotationOverContentViewController = OTFullScreenAnnotationViewController()
    ...
    override func viewDidLoad() {
        ...
        statusButton.addTarget(self, action: #selector(AnnotationOnScreenViewController.stopAnnotation), for: .touchUpInside)
        statusButton.backgroundColor = UIColor(red: 118.0/255.0, green: 206.0/255.0, blue: 31.0/255.0, alpha: 1.0)
        statusButton.setTitle("You are annotating your screen, Tap here to dismiss", for: UIControlState.normal)
    ...
    @IBAction func viewPressed(_ sender: UIView!) {
        holderView.bringSubview(toFront: sender)
    }
